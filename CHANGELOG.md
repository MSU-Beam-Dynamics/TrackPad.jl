...
- JuTrack is no longer a test dependency. The parity suite compares against
  frozen JuTrack output in `test/jutrack_reference.jl`, regenerated by
  `test/jutrack_reference/generate.jl` (the only file that uses JuTrack); every
  assertion and tolerance is unchanged, and `verify_elements.jl` fails if a
  parity case and the recorded data drift apart. PolySeries is now the package's
  only unregistered dependency. See `test/jutrack_reference/README.md`.
...
- `Lattice` recursed until it overflowed the stack when given a collection whose
  ...
  type, and rejects a non-element with an `ArgumentError` naming the offending
  types.
