...
@testset "TrackPad Enzyme Compatibility" begin
    ...
    # exact-Hamiltonian drift is differentiated directly. The frozen JuTrack
    # finite-difference references were taken on the same convention, so they
    # only apply while that constant is true.
    ...
    function enzyme_fwd_derivative(f::Function, x::Float64)
        ...
        deriv = out isa Tuple ? out[1] : out
        while !(deriv isa Number)
            if deriv isa NamedTuple
                deriv = first(values(deriv))
            elseif deriv isa Tuple
                deriv = deriv[1]
            else
                error("Unexpected Enzyme derivative container: $(typeof(deriv))")
            end
        end
        return deriv
    end
    ...
    # JuTrack: DRIFT(1.0), SBEND(len=0.4, angle=0, PolynomB=[0, k, 0, 0]),
    # DRIFT(1.0), same central difference in k about 0.6.
    fd_static_jt = jutrack_reference("enzyme/fd_static")
    ...
    # Same JuTrack line, differenced in the time that sets k = 0.6 + 0.15 sin(t).
    fd_time_jt = jutrack_reference("enzyme/fd_time")
